...
**当前版本：`v2.7.0`** · [更新内容 →](#-v270-更新内容) · [完整历史 →](CHANGELOG.zh-CN.md)
...
**「手把手带你」版本** —— 路由器内置零输入新手教程：

- **教程模式。** 输入 `/money 新手`（或 `/money onboarding`）即可获得约 5 分钟的引导式演练，全程零假设：3 句话定向、直接回车也能继续的演示角色选择器、现场跑一遍「发现 → 策略 → 行动」迷你演示，每步跟一行「💡 刚才发生了什么」白话解释。
- **演示版商业简报。** 每次教程都以一份 ≤40 行的真实交付物收尾——切入点、带价格的真实竞品、建议定价、最大风险假设、7 天计划——新手先看到真价值，再被问任何问题。
- **明确要求永远优先。** 教程触发词在模式检测之前识别、优先级高于已保存状态——老用户要教程就给教程，不会被恢复提示拦截。
- **反模式护栏。** 不先要邮箱、不在演示里跑完整技能、不对新手甩 25 技能大表、不问开放式问题。
