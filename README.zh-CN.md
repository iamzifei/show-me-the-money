...
**当前版本：`v2.7.1`** · [更新内容 →](#-v271-更新内容) · [完整历史 →](CHANGELOG.zh-CN.md)
...
- **套件有中文名了：来财。** 两个字，自带梗，好记好传——现在直接说「来财」就能唤起 `/money`。
- **按 OECD skill 审查原则（@dontbesilent `/dbs-skill-cleaner` 同款标准）完成自审，所有发现全部修复：**
  - 每处 ccapi.ai 推荐都加上了明确的利益披露——它是本套件作者自己的产品，技能在提供该选项时必须说明。
  - 移除了泄漏进 `/money-product` 的作者私有 Vercel team scope（其他用户照跑会直接部署失败）。
  - 价值量化里的「没有这个技能你会怎样」从必填降级为「有真实依据才写」——禁止编造反事实损失来夸大价值。
  - 新增[隐私与披露](#-隐私与披露)章节：所有数据只存本地，套件唯一的网络调用是 npm 版本检查。
