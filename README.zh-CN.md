...
**「内容手艺」版本** —— `/money-content` 与 `/money-social` 吸收一套实战验证的短视频内容方法论，泛化到所有内容形态：

- **选题闸门。** 动笔前每个题先过供需测试：服务谁的痛、谁已经在做、以及「头部 5 个结果已经讲清了——你好在哪？」（合法答案只有三种：一手数据、反共识但正确、把复杂讲成大白话）。一篇内容约 80% 的表现在选题时已锁定。
- **逐句信息密度评分。** 每句话对照校准锚点打分（80+ = 别处听不到的一手结论；<40 = 废信息），按形态设达标线；修法优先级：先砍废话，再展开被埋没的高分点。
- **唯一核心共鸣检查。** 说六件事等于什么都没说——找出唯一核心，对照四种失效模式（核心稀释 / 机制没说清 / 立场漂移 / 正确但平淡），替读者说出他们私下相信但不敢公开说的那句话。
- **卖真红线 + 钩子手艺。** 不编案例不编数字；钩子先分型（数据型 / 情绪型）再用对的尺子评；候选 3-6 个真不同，禁 10+。
- **Content MVP 测试阶梯。** 未验证的题先发一条零成本文字帖再投产视频：收藏率对照自己近期中位数，决定升级（短帖 → thread → 博客 → 视频 → 付费投放）还是记录死因毙题。

### 历史版本？

- [v2.7.1](CHANGELOG.zh-CN.md#v271--2026-07-17) —— 中文名「来财」+ OECD 透明度整改（利益披露、隐私章节、有据价值声明）
- [v2.7.0](CHANGELOG.zh-CN.md#v270--2026-07-17) —— 教程模式：零输入新手引导（`/money 新手` / `/money onboarding`）
- [v2.6.0](CHANGELOG.zh-CN.md#v260--2026-07-07) —— 双模式路由器：任务后导航、静默语言检测、快速路由、主动交接
...
（也可以：`/money 教程`、`/money onboarding`、`/money tutorial`）

一个 5 分钟的傻瓜式引导教程，**不需要你输入任何东西** —— 哪怕你完全不知道该打什么字，它会自己挑一个示例生意，现场跑一遍「发现商机 → 制定策略 → 行动计划」的迷你演示，产出一份真实可用的**演示版商业简报**，并教会你唯一需要记住的 3 条命令。[看看它长什么样 →](#-完全新手--傻瓜式引导教程money-新手)
...
### 🎓 完全新手 — 傻瓜式引导教程（`/money 新手`）

不知道该输入什么？这个模式什么都不假设、什么都不要求：

```
/money 新手          （或：/money 教程、/money onboarding、/money tutorial）
```

教程实际会做的事：

1. **快速定向** —— 用 3 句话讲清整个系统（创意 → 策略 → 产品 → 客户 → 自动驾驶；你只需要记住 `/money` 这一条命令）
2. **选一个演示角色** —— 编号选项 + 默认值，直接回车也能继续：
   - 👩‍💻 每周有 10 小时的开发者 · ✍️ 有 3k 粉丝的内容创作者 · ☕ 本地咖啡店老板 · 🙋 或者用你自己的真实情况
3. **现场迷你演示** —— 压缩版的 `/money-discover` → `/money-strategy` → 7 天行动计划，每步之后跟一行「💡 刚才发生了什么」的白话解释
4. **毕业交接** —— 只问一个 yes/no：「要不要用你自己的真实情况把这套流程跑一遍？」

输出样例（角色 #1，节选）：

```
📋 演示版商业简报 —— 「截图转发票」微型 SaaS

🎯 切入点：每月给 5-15 个客户开发票的自由设计师，每月浪费约
   2 小时手动重新录入条目 —— 他们已经在为更差的替代方案付费
   （人肉 VA、难用的 OCR 应用）。

💰 竞品与定价：
   - Invoice Ninja（免费-$10/月，通用工具，没有截图流程）
   - Hectic（$15/月，自由职业者套件，发票只是附带功能）
   - Upwork 上雇 VA（约合 $40/月）
   → 建议定价：$12/月 —— 低于套件价，高于「玩具」价

⚠️ 最大风险假设：设计师是否敢不逐行检查就把 OCR 结果直接发给客户。

📅 7 天计划：第 1 天（< 1 小时）—— 把一句话 pitch 发到两个
   自由设计师社群，数一下反应。第 2-7 天：落地页 → 等待名单 →
   5 个问题访谈 → 假门演示 → 做决定。
...
💡 完整教程约 5 分钟，全程不问开放式问题 —— 每个提问都有编号选项和默认值。
...
- **所有数据只存本地。** 套件收集或生成的一切状态——画像、learnings、会话检查点、使用记录——全部以明文 markdown/JSONL 存在你机器的 `~/.smtm/` 下，永不上传。唯一的网络调用是静默的 npm 版本检查（`npm view`），断网自动跳过。
- **随时可清除。** 删除 `~/.smtm/` 即抹掉全部收集数据。
- **利益披露。** 技能在需要 AI API key 时可能会提到 [ccapi.ai](https://ccapi.ai) 作为便捷选项。ccapi.ai 是本套件作者自己的产品，永远不是必需——你自己的 API key 始终可用。
