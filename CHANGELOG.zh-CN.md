...
**「内容手艺」版本。** `/money-content` 与 `/money-social` 吸收一套实战验证的短视频内容方法论，并泛化到套件产出的所有内容形态。没有新增技能文件——总数保持 25 个。

### 新增
- **Stage 1.5 选题闸门（供需测试）。** 每个候选选题入日历前过四关：需求（服务谁的痛——"介绍我们产品"即毙题信号）、供给、认知落差自检（"头部 5 个结果已经讲清了——你好在哪？"合法答案只有三种：一手数据、反共识但正确、把复杂讲成大白话）、双受众规则。一篇内容约 80% 的表现在选题时已锁定。
- **通用写作铁律**（覆盖所有形态）：特写镜头规则（具象胜抽象）、大白话规则（需要解释的词都不是好文案；行话缩小算法分发池）、18 秒标准（每个点给可直接照做的动作）、**卖真红线**（不编案例、不编数字、不编个人细节——没素材就要真事）、一篇一核心（7±2）、密度即"短"的本义。
- **钩子公理 + 钩子分型。** 陈述句不自问自答、最强信号顶前 2 秒、3-6 个真不同候选并标注技法（禁 10+）、一次只改一个变量。评钩子先分型——数据型按信息缺口和实证评，情绪型按未消解张力和共鸣精度评——稿型适配高于钩子分，混合体常胜。
- **Stage 4.2 唯一核心共鸣检查。** 结构级诊断：列出全部主张 → 找唯一核心（全面 ≠ 专业）→ 对照四种失效模式（核心稀释 / 机制没说清 / 立场漂移 / 正确但平淡）→ 一句话输出修复后的骨架。
- **Stage 4.3 逐句信息密度评分。** 校准锚点（80+ = 别处听不到的一手结论；60-79 = 具体可执行；40-59 = 真实但常见；<40 = 废信息），按形态设达标线（短内容 ≥8 句 60+ 且 ≥4 句 80+），修法优先级（先砍废话，再展开只提了概念的高分点），钩子/转场句误伤防护，默认只诊断、低分必引原句。
- **Content MVP 测试阶梯。** 未验证的题先发文字帖再投产：提炼一句话核心主张 → 低成本发出 → 48-72h 对照自己近期中位数看收藏率 → 验证则沿阶梯升级（短帖 → thread → 博客 → 视频 → 付费投放），平了就毙题并经 `/money-learn` 记录死因；每次判定附受众匹配的信号折扣声明。
- **标题清单新增三铁律**：留悬念不自答、话题扩大不缩窄（普世词替行话）、击中表象之下的真痛点。
- **`/money-social`**：共享钩子公理、结果型钩子的卖真规则、指标表新增收藏率行（质量信号）、test-before-invest 入口指向 MVP 阶梯。
