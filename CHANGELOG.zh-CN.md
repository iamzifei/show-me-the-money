...
**「来财」版本。** 中文名 + 一轮按 OECD skill 审查原则（@dontbesilent `/dbs-skill-cleaner` 同款标准）自审后的透明度整改。

### 新增
- **中文名：来财。** 已进入双语 README 标题，「来财」也成为 `/money` 的触发词。
- **隐私与披露章节**（双语 README）：所有收集的状态（画像、learnings、检查点、使用记录）只存本地 `~/.smtm/`，永不上传；套件唯一的网络调用是 npm 版本检查；删除 `~/.smtm/` 即清除全部数据。`/money` 的 telemetry 步骤现在显式声明这一不变式。

### 变更
- **ccapi.ai 利益披露。** 两处提供 ccapi.ai 作为 API key 便捷选项的位置（`/money`、`/money-product`）现在都披露 ccapi.ai 是本套件作者自己的产品，且技能在提供该选项时必须附带披露。
- **「没有这个技能你会怎样」从必填降级为可选且必须有据。** 价值量化模板不再强制编写反事实失败叙事；只有当会话中存在真实依据时才可写，明令禁止编造反事实。

### 修复
- **移除泄漏进 `/money-product` 的作者私有 Vercel team scope（`--scope orris`）。** 其他用户照此指令部署会直接失败。

---

## v2.7.0 — 2026-07-17

**「手把手带你」版本。** 路由器内置零输入新手教程——没有新增技能文件，总数保持 25 个。

### 新增
- **教程模式（`/money 新手` / `/money onboarding`）。** 为完全不知道该输入什么的新手准备的约 5 分钟引导式演练。触发词（`/money tutorial`、`/money 教程`、`/money 入门`、`/money 上手`、"这个怎么用"、"how do I use this"）在模式检测**之前**识别，优先级高于已保存状态——明确要教程就给教程，老用户带存档也不例外。
- **五幕流程，全程零必填输入。** 3 句话定向；演示角色选择器（开发者 / 创作者 / 咖啡店老板 / 你的真实情况——空回复或看不懂的回复一律默认 #1）；现场迷你演示串起压缩版的发现 → 策略 → 7 天行动计划，每步跟一行「💡 刚才发生了什么」把演示挂回真实技能；三命令记忆闭环（`/money`、`/money-save`、`/money-restore`）；最后只问一个 yes/no 完成毕业交接进入真实流水线。
- **演示版商业简报。** 每次教程都把演示产出汇成一份 ≤40 行的简报（切入点、带价格的真实竞品、建议定价、最大风险假设、7 天计划）——教程的价值证明物。
- **反模式护栏。** 教程明确禁止：先要邮箱/社媒再给价值、在演示里跑完整技能、对新手甩 25 技能大表、开放式提问、不出简报就结束。
- **README（英文 + 中文）。** 快速开始新增新手入口，使用示例开头新增带节选输出样例的完整示例。
